...
class Check(Base, AuditMixin, SoftDeleteMixin, TimestampMixin, UUIDMixin, SFIDMixin):
    ...
    __table_args__ = (
        ...
        Index("idx_checks_created_by_id", "created_by_id"),
        Index("idx_checks_last_updated_by_id", "last_updated_by_id"),
        CheckConstraint(
            "date_completed <= CURRENT_DATE", name="chk_date_completed_not_future"
        ),
        CheckConstraint(
            "number_of_trainings_attended <= 50 AND number_of_trainings_attended >= 0",
            name="chk_number_of_trainings_attended_valid",
        ),
        {"schema": SCHEMA},
    )
    ...
    observation_id = Column(
        UUID(as_uuid=True), ForeignKey(f"{SCHEMA}.observations.id"), nullable=True
    )
    farm_visit_id = Column(
        UUID(as_uuid=True), ForeignKey(f"{SCHEMA}.farm_visits.id"), nullable=True
    )
    training_session_id = Column(
        UUID(as_uuid=True), ForeignKey(f"{SCHEMA}.training_sessions.id"), nullable=False
    )
    check_type = Column(
        Enum("Training Observation", "Farm Visit", name="check_type_enum", schema=SCHEMA),
        nullable=False,
    )
    ...
    attended_last_months_training = Column(
        Enum(
            "Yes",
            "No",
            "No training was offered",
            name="attended_last_months_training_enum",
            schema=SCHEMA
        ),
        nullable=False,
    )
    ...
    checker = relationship("User", back_populates="checks", foreign_keys=[checker_id])
