...
class WetmillVisit(Base, SoftDeleteMixin, TimestampMixin, UUIDMixin, AuditMixin):
    ...
    visiting_staff_id = Column(UUID(as_uuid=True), ForeignKey(f"{SCHEMA}.users.id"), nullable=True)
    ...
    submitter = relationship("User", back_populates="wetmill_visits", foreign_keys=[visiting_staff_id])
