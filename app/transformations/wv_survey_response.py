...
class WVSurveyResponseTransformer:
    ...
    def transform(
        self, payload: Dict, survey_type: str, content: dict
    ) -> WVSurveyResponseCreate:
        """Transform CommCare payload to WVSurveyResponseCreate schema"""
        try:

            session_data = self._map_wv_survey_response_data(
                payload, survey_type, content
            )
            ...
            return WVSurveyResponseCreate(form_visit_id=form_visit_id, **session_data)
    ...
    def _map_wv_survey_response_data(
        self, payload: Dict, survey_type: str, content: dict
    ) -> Dict[str, Any]:
        """Map observation result data"""
        form_id = payload.get("id")
        date = payload.get("form", {}).get("date", "")
        return {
